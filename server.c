...
int answerTimeout = 30;
float marksForCorrectAnswer = 1.0;
float marksDeductedForWrongAnswer = 0.25;
...
typedef struct {
    ...
    char correct;
    char padding1[3]; // Align to 4-byte boundary
    int difficulty;
} Question;
...
void load_questions() {
    ...
    while (qIndex < MAX_QUESTIONS) {
        ...
        if (q.question[0] == '\0' || q.optionA[0] == '\0' || q.optionB[0] == '\0' ||
            q.optionC[0] == '\0' || q.optionD[0] == '\0' ||
            !strchr("ABCD", q.correct) || q.difficulty < 1 || q.difficulty > 3) {
...
void send_exam_data(int client_sock) {
    ...
    // Reinitialize to prevent corruption
